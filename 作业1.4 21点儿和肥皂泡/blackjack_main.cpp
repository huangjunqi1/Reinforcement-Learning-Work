...
class BlackjackPolicyLearnableDefault : public BlackjackPolicyBase{
    ...
    public:
        ...
        void update_policy(){
            // TODO
            for (int dealer_shown = 1;dealer_shown<11;dealer_shown++)
                for (int player_ace = 0; player_ace<2; player_ace++)
                    for (int player_sum = 12;player_sum<=21;player_sum++){
                        policy[dealer_shown][player_ace][player_sum] = (value[dealer_shown][player_ace][player_sum][0]>value[dealer_shown][player_ace][player_sum][1])?0:1;
                    }
            // simply take argmax
        }
        ...
        void update_value(Blackjack& env, int n=10000){
            ...
            for (int dealer_shown = 1;dealer_shown<11;dealer_shown++)
                for (int player_ace = 0; player_ace<2; player_ace++)
                    for (int player_sum = 12;player_sum<=21;player_sum++){
                        if (player_ace == 0 && player_sum == 21)
                        { 
                            policy[dealer_shown][player_ace][player_sum] = Blackjack::STICK;
                            continue; //invailid state
                        }
                        auto first_action = Blackjack::HIT;
                        ...
                        while (!done){
                            ...
                            int action;
                            if (state.turn == Blackjack::DEALER){
                                action = state.dealer_sum >= 17 ? Blackjack::STICK : Blackjack::HIT;
                            } else {
                                action = policy[state.dealer_shown][state.player_ace][state.player_sum];
                            }
                            result = env.step(action);
                            done = result.done;
                        }
                        value[dealer_shown][player_ace][player_sum][first_action] = (value[dealer_shown][player_ace][player_sum][first_action]*iter + result.player_reward)/(iter+1);
                        
                         
                        
                        
                        
                        first_action = Blackjack::STICK;
                        env.reset(dealer_shown,player_ace,player_sum);
                        result = env.step(first_action);
                        done = result.done;
                        while (!done){
