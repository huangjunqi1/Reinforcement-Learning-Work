...
class WindyGridWorldPolicySarsa : public WindyGridWorldPolicyBase{
    public:
        ...
        void learn(int iter = 1000000){
            srand(19260817);
            for (int episode = 0; episode<iter; episode++){
                WindyGridWorld env = WindyGridWorld();
                env.verbose = false;
                env.reset();
                auto state = env.state();
                auto action = epsilon_greedy(state);
                while (!env.done()){
                    auto ret = env.step(action);
                    auto new_state = ret.first;
                    auto reward = ret.second;
                    auto new_action = epsilon_greedy(new_state);
                    q[state.second][state.first][action]+=  \
                        alpha*(reward + gamma*q[new_state.second][new_state.first][new_action] - q[state.second][state.first][action]);
                    state = new_state;
                    action = new_action;
                }
            }
    ...
    private:
        ...
        int epsilon_greedy(const WindyGridWorld::State& state){
            if (((((long long)rand())<<16)+rand()) % 100000 < epsilon * 100000){
                return rand() % 4;
            }
            return (*this)(state);
        }
...
class WindyGridWorldPolicyQLearning : public WindyGridWorldPolicyBase{
        ...
        void learn(int iter = 1000000){
            ...
            for (int episode = 0; episode<iter; episode++){
                ...
                while (!env.done()){
                    ...
                    auto new_state = ret.first;
                    auto reward = ret.second;
                    auto new_action = (*this)(new_state);
                    q[state.second][state.first][action]+=  \
                        alpha*(reward + gamma*q[new_state.second][new_state.first][new_action] - q[state.second][state.first][action]);
                    state = new_state;
                    action = new_action;
    ...
    private:
        ...
        double epsilon, alpha, gamma;
        int epsilon_greedy(const WindyGridWorld::State& state){
            if (((((long long)rand())<<16)+rand()) % 100000 < epsilon * 100000){  //修改了随机函数
                return rand() % 4;
            }
            return (*this)(state);
        }
